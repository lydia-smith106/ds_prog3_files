...
int main()
{
	...
	//dictHash.insertItem("hello");
	//dictHash.insertItem("practice");
	int n = dictHash.returnSize();
	cout << "the size is : " << n <<endl;
	dictHash.displayHash();
	...
	if (bookfile.is_open())
	{
		while (!bookfile.eof())
		{
			...
			else if (newWord.empty() == false)
			{
				...
				if (dictHash(newWord) == true)
				{
					found++;
					//found_compares = found_compares + dict_list.getCompares();
